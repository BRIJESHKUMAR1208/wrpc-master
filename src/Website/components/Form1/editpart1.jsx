...
export const Editformonepart1 = () => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (!selectedFile) {
        formDataToSend.append("FIR_Spath", formData.fir_spdfpath);
      } else {
        formDataToSend.append("FIR_S", selectedFile);
      }
      if (!selectedFile1) {
        //setSelectedFile1(formData.dr_s)
        formDataToSend.append("DR_Spath", formData.fir_spdfpath);
      }
      else {
        formDataToSend.append("DR_S", selectedFile1);
      }
      if (!selectedFile2) {
        formDataToSend.append("EL_Spath", formData.el_spath);
      }
      else {
        formDataToSend.append("EL_S", selectedFile2);
      }
      if (!selectedFile3) {
        formDataToSend.append("TR_Spath", formData.tr_spath);
      }
      else {
